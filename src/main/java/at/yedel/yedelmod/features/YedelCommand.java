...
@Command(
    ...
    customHelpMessage = logo + " §eUnknown subcommand, refer to the command index (/yedel).",
    chatColor = ChatColor.BLUE
)
public class YedelCommand {
    private static final UTextComponent formattingCodes =
        new UTextComponent(
    ...
    private static final UTextComponent formattingGuideMessage = new UTextComponent(logo + " §e§nHover to view the formatting guide.")
    ...
    public static class Ping {
        ...
        @SubCommand(description = "Sends a tab completion packet and waits for the response. Works on all servers.",
            aliases = "t"
        )
        public void tab() {
        ...
        @SubCommand(description = "Sends a statistics packet and waits for the response. Works on all servers.",
            aliases = "s"
        )
        public void stats() {
        ...
        @SubCommand(description = "Uses the Hypixel ping packet and waits for the response. Only works on Hypixel.",
            aliases = "h"
        )
        public void hypixel() {
    ...
    @SubCommand(description = "Shows your total playtime (while playing on servers) in hours and minutes.",
        aliases = "pt"
    )
    ...
    @SubCommand(description = "Simulates a chat message, also supports color codes with ampersands (&).",
        aliases = "simc"
    )
    ...
    @SubCommand(description = "Shows messages from me about the mod. These can be anything from tips to bug notices.",
        aliases = "message"
    )
    public void yedelmessage() {
        new Thread(() -> {
            try {
                String yedelMessage = Requests.getJsonObject(new URL("https://yedelo.github.io/yedelmod.json")).
                    getAsJsonObject().
